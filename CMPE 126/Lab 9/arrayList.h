...
public:
    ...
    int seqSearchRec(T element, int start = 0) {
        if (start >= length) {return -1;} // Return -1 if element is not found
        if (list[start] == element) {return start;}
        return seqSearchRec(element, start + 1);
    }
};
